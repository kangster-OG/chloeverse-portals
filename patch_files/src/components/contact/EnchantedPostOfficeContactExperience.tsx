...
/** -------- Minimal WebAudio SFX (silent fail if blocked) -------- */
...
function tone(sfx: Sfx, freq: number, durMs: number, type: OscillatorType = "sine", gain = 0.7) {
...
function waxPop(sfx: Sfx) {
  tone(sfx, 310, 70, "triangle", 0.75);
  tone(sfx, 520, 60, "sine", 0.65);
  tone(sfx, 210, 120, "square", 0.35);
}
...
export function EnchantedPostOfficeContactExperience() {
  ...
  const close = () => {
    const sfx = ensureSfx();
    if (sfx) tone(sfx, 420, 70, "triangle", 0.5);
    setOpenLetter(false);
  };

  const openExternal = (href: string) => {
    const sfx = ensureSfx();
    if (sfx) tone(sfx, 860, 55, "sine", 0.55);
    window.open(href, "_blank", "noopener,noreferrer");
  };

  const copy = async (text: string) => {
    const sfx = ensureSfx();
    if (sfx) tone(sfx, 980, 55, "square", 0.32);
    try {
  ...
  return (
    ...
    >
      <style jsx global>{`
        ...
        @keyframes candleFlicker { 0%,100% { opacity: .45; transform: translateY(0px) scale(1); } 50% { opacity: .75; transform: translateY(-1px) scale(1.02);} }
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(900px 700px at 35% 35%, rgba(255,200,120,0.14) 0%, rgba(255,200,120,0.0) 58%), radial-gradient(700px 500px at 70% 35%, rgba(140,255,220,0.08) 0%, rgba(140,255,220,0.0) 62%), linear-gradient(180deg, #05060a 0%, #060711 45%, #03040a 100%)",
        }}
      />
      <div
        className="absolute inset-0"
        style={{
          background:
            "radial-gradient(900px 560px at 50% 40%, rgba(0,0,0,0) 0%, rgba(0,0,0,0.70) 70%, rgba(0,0,0,0.92) 100%)",
        }}
      />

      {/* Candle glows */}
      <div className="absolute left-[8%] top-[34%] h-[180px] w-[180px] rounded-full blur-2xl"
        style={{ background:"radial-gradient(circle, rgba(255,200,120,0.22) 0%, rgba(255,200,120,0.0) 68%)", animation: reducedMotion ? "none" : "candleFlicker 2.8s ease-in-out infinite" }}
      />
      <div className="absolute right-[10%] top-[30%] h-[220px] w-[220px] rounded-full blur-2xl"
        style={{ background:"radial-gradient(circle, rgba(180,120,255,0.14) 0%, rgba(180,120,255,0.0) 70%)", animation: reducedMotion ? "none" : "candleFlicker 3.2s ease-in-out infinite" }}
      />
      ...
      <div className="absolute left-0 right-0 top-0 z-10 px-6 pt-6">
        <div className="mx-auto flex w-full max-w-[1200px] items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[14px] font-semibold tracking-wide text-white/85">Enchanted Post Office</div>
        ...
        <div className="relative mx-auto h-[84px] rounded-[22px] bg-[#0f1118]/85 ring-1 ring-white/10 shadow-[0_30px_90px_rgba(0,0,0,0.65)]">
          ...
          {!openLetter && (
            <div
              ...
              style={{
                background:
                  "radial-gradient(closest-side, rgba(255,200,120,0.24) 0%, rgba(255,200,120,0.0) 70%)",
                filter: "blur(0.2px)",
      ...
      <div className="absolute inset-0">
        ...
        >
          <div className="absolute inset-0 rounded-[18px] opacity-60"
            style={{ background: "radial-gradient(circle at 20% 0%, rgba(255,255,255,0.55) 0%, rgba(255,255,255,0.0) 52%)" }}
          />
        ...
        >
          <div className="absolute inset-0 rounded-[18px] opacity-60"
            style={{ background: "radial-gradient(circle at 80% 0%, rgba(255,255,255,0.5) 0%, rgba(255,255,255,0.0) 52%)" }}
          />
          ...
          <div className="relative grid gap-4 px-7 py-6 md:grid-cols-[0.9fr_1.1fr]">
                ...
                {CHANNELS.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        const sfx = ensureSfx();
                        if (sfx) tone(sfx, isActive ? 520 : 760, 55, "sine", 0.45);
                        setActive(c.key);
                      }}
                      className={[
                    ...
                    >
                      {/* perforation */}
                      <div className="absolute inset-y-0 left-0 w-[10px] opacity-40"
                        style={{
                          background:
            ...
            <div className="rounded-[22px] bg-white/55 p-6 ring-1 ring-black/10">
              <div className="flex items-center justify-between gap-3">
                ...
                {copied && (
                  <div className="rounded-full bg-black/5 px-3 py-1 text-[12px] text-black/60 ring-1 ring-black/10">
                    {copied}
                  </div>
                )}
              </div>
              ...
              <div className="mt-2 text-[14px] leading-relaxed text-black/60">
                Tap <span className="font-semibold text-black/70">Open</span> to connect, or{" "}
                <span className="font-semibold text-black/70">Copy</span> to save it.
              </div>
      ...
      {/* Quill cursor + ink trail (disabled for reduced motion) */}
      {!reducedMotion && (
        <>
          {/* ink trail */}
          ...
          >
            <div
              className="relative h-[22px] w-[22px] rotate-[-18deg]"
              style={{
