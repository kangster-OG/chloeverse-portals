...
import CollabsReelsCorridor from "./CollabsReelsCorridor";
...
function MuseumScene({
  ...
  const galleryCamera = useMemo(() => new THREE.Vector3(0.08, 1.22, 2.5), []);
  const fromTarget = useMemo(() => new THREE.Vector3(0, 1.2, 0), []);
  const toTarget = useMemo(() => new THREE.Vector3(0.2, 1.1, -1.2), []);
  const galleryTarget = useMemo(() => new THREE.Vector3(0, 1.1, -1.8), []);
...
export default function CollabsShell({ children }: { children: React.ReactNode }) {
  ...
  const isReelsRoute = pathname === "/collabs/reels";
  ...
  return (
    <main className="relative h-screen w-full overflow-hidden bg-black text-white">
      ...
      {!isReelsRoute ? (
        ...
        <CollabsReelsCorridor smokeAction={smokeMode ? smokeAction : null} />
      )}
